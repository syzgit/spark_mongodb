...
object getAndPutmongodata {
  def main(args:Array[String]){
    ...
    config.set("mongo.input.uri", "mongodb://spider:Serwe-8dfgre@120.26.41.22:27017/cr_data.finance.juchao.item")
...
val RDD = sc.textFile("/user/yzsun/sun_result_1/1").map { x =>
     ...
     var a=0
        ...
        val first = (splits(0).trim()).indexOf("_")
        val split0 = splits(0).trim().subSequence(0, first)
      query.append("_id","ObjectId("+split0+")")
