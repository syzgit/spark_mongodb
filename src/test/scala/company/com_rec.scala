...
object com_rec {
  def main(args:Array[String])
  {
    ...
    val sqlContext = new SQLContext(sc)
     var srcRDD = sc
  .textFile("C:/Users/yzsun.abcft/Desktop/company/2017-07-24stock.txt")
  .flatMap (line =>
            line.split(",")).map(word => (word.trim(), 1))  
          val ida_result =  srcRDD.groupBy(_._1).map {  
     case (word, list) => (word, list.size)  
   }  
                    val writer = new PrintWriter(new File("C:/Users/yzsun.abcft/Desktop/company_test_result/25.txt"))
